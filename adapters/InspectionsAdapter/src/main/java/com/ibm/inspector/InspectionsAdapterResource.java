...
@Path("/inspections")
public class InspectionsAdapterResource {
	/*
	...
	static Logger logger = Logger.getLogger(InspectionsAdapterResource.class.getName());
	...
	/*
	 * Path for method:
	 * "<server address>/mfp/api/adapters/InspectionsAdapter/inspections"
	 */	
...
//	@ApiResponses(value = { @ApiResponse(code = 200, message = "Hello message returned") })
	...
	public Response getInspections(){
		try {
			...
			InspectionsAdapterResource._inspections = (JSONArray)JSON.parse(is);
		} catch (IOException ex){
			logger.log(Level.SEVERE, "Could not load Appointments.json", ex);
			return Response.status(Status.INTERNAL_SERVER_ERROR).build();
		}
		return Response.ok(InspectionsAdapterResource._inspections).build();
	}	
	...
	public Response createAppointment(JSONObject appointment){
		...
		InspectionsAdapterResource._inspections.add(appointment);
	...
	public Response updateAppointments(String appointmentsStr){
		...
		InspectionsAdapterResource._inspections = appointments;
	...
	public Response updateAppointment(JSONObject appointment, @PathParam("appointmentId") long id){
		...
		if (index > -1){
			InspectionsAdapterResource._inspections.set(index, appointment);			
		} else {
	...
	public Response createMedicalTest(JSONObject medicalTest, @PathParam("appointmentId") long id){
		...
		JSONObject appointment = (JSONObject)InspectionsAdapterResource._inspections.get(appointmentIndex);
	...
	private Response _loadAppointments() {
		try {
			if (InspectionsAdapterResource._inspections == null){
	...
	private int _findAppointmentIndex(long id){
		for (int i=0; i< InspectionsAdapterResource._inspections.size(); i++){
			if (Long.valueOf(id).equals(((JSONObject)InspectionsAdapterResource._inspections.get(i)).get("id"))){
				return i;
			}
		}
		return -1;
...
//	/*
//	 * Path for method:
//	 * "<server address>/mfp/api/adapters/InspectionsAdapter/resource/greet?name={name}"
//	 */
//
...
//	/*
//	 * Path for method:
//	 * "<server address>/mfp/api/adapters/InspectionsAdapter/resource/{path}/"
//	 */
//
...
//	/*
//	 * Path for method:
//	 * "<server address>/mfp/api/adapters/InspectionsAdapter/resource/prop"
//	 */
//
...
//	/*
//	 * Path for method:
//	 * "<server address>/mfp/api/adapters/InspectionsAdapter/resource/unprotected"
//	 */
//
...
//	/*
//	 * Path for method:
//	 * "<server address>/mfp/api/adapters/InspectionsAdapter/resource/protected"
//	 */
//
